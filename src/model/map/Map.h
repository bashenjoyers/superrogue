...
static std::uniform_int_distribution<int>
    drop_gen_i(0, superrogue::values::items_types.size() +
                      superrogue::values::potions_types.size() - 1);
...
struct MapInfo {
  ...
  MapInfo(
      std::vector<superrogue::abstract::MapEntityWithPosition> map_positions,
      superrogue::map::PersonWithPosition person);
};
...
class Map {
  ...
  std::map<superrogue::abstract::Position, superrogue::game_object::item::IItem>
      items;
  superrogue::abstract::GameStatus game_status =
      superrogue::abstract::GameStatus::NEXT_LVL;
  ...
  superrogue::abstract::MapEntity
  get_cell_type(superrogue::abstract::Position pos) const noexcept;
  std::vector<superrogue::abstract::MapEntityWithPosition> visible_cells(
      const superrogue::abstract::Position &pos, int radius, bool ignore_walls,
      const std::vector<superrogue::abstract::Position> &area) const noexcept;
  std::optional<superrogue::game_object::item::IItem>
  drop_item() const noexcept;
  void change_item();
  bool any_step_anybody(WithPosition anybody,
                        superrogue::abstract::Position pos) noexcept;
  bool step_anybody(superrogue::game_object::character::CharacterAction action,
                    WithPosition anybody) noexcept;
  bool
  action_person(superrogue::game_object::character::CharacterAction action);
  void action_enemy(superrogue::game_object::character::CharacterAction action,
                    EnemyWithPosition enemy);
  void
  punch_cells_in_order(std::vector<superrogue::abstract::Position> positions,
...
public:
  Map(std::set<superrogue::game_object::character::Enemy> enemies,
      superrogue::game_object::character::Person person, MapOptions map_options,
      int level);
