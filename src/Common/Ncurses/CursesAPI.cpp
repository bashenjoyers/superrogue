...
void NcursesWindow::drawElement(char c, size_t x, size_t y, bool highlighted) {
  ...
  mvwaddch(win, bordersOffset + y, bordersOffset + x,
           c); // TODO handle out of range
  if (highlighted)
...
NcursesWindow::NcursesWindow(WINDOW *_win, size_t h, size_t w, size_t x,
                             size_t y)
    : win(_win), h(h), w(w), x(x), y(y) {}
...
NcursesWindow::NcursesWIndowPtr NcursesAPI::getWindow(size_t h, size_t w,
                                                      size_t x, size_t y) {
...
size_t NcursesAPI::getXsize() { return getmaxx(stdscr); }

size_t NcursesAPI::getYsize() { return getmaxy(stdscr); }

int NcursesAPI::getKey() { return getch(); }
