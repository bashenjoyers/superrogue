...
void CursesFactory::resync() {
  ...
  inf = TerminalInfo{.w = ncurses->getXsize(), .h = ncurses->getYsize()};
...
std::shared_ptr<CursesInventoryRenderer> CursesFactory::getInventoryRenderer() {
  if (inventoryRenderer == nullptr)
    inventoryRenderer =
        std::make_shared<CursesInventoryRenderer>(getInventoryWindowConfig());
  return inventoryRenderer;
}

WindowConfig CursesFactory::getMapWindowConfig() {
  return WindowConfig{.width =
                          (size_t)((float)inf.w - (float)inf.w * invMapCoef),
                      .height = inf.h,
...
WindowConfig CursesFactory::getInventoryWindowConfig() {
  return WindowConfig{.width = (size_t)((float)inf.w * invMapCoef),
                      .height = inf.h,
                      .xPos = 0,
                      .yPos = 0};
}
...
WindowConfig CursesFactory::getMainMenuWindowConfig() {
  return WindowConfig{.width = inf.w, .height = inf.h, .xPos = 0, .yPos = 0};
}
} // namespace View