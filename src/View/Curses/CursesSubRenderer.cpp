...
void CursesInventoryRenderer::render(RenderInfo info) {
  ...
  for (size_t curRow = 0; curRow < maxPotions; curRow++) {
      if (curRow < potions.size()) renderPotion(potions[curRow], curRow, curRow == info.cursorState->getPotionsCursor());
        else renderEmptyPotion(curRow, curRow == info.cursorState->getPotionsCursor());
  }
