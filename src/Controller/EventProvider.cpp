...
EventProvider::EventProvider(std::shared_ptr<CursorState> cursorState,
  ...
  keyMapping = {
      ...
      {'k', std::make_shared<StepBackModelEvent>(map)},
      {'i', std::make_shared<StepForwardModelEvent>(map)},
      {'j', std::make_shared<StepLeftModelEvent>(map)},
      {'l', std::make_shared<StepRightModelEvent>(map)},
